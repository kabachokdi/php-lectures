...
$route = $_GET['route'] ?? 'task/list'; //определение маршрута

$config = require __DIR__ . '/../config.php'; //загрузка конфига
...
//динамическая привязка интерфейса к реализации
...
$container->set(TaskController::class, function (Container $c) {
    $repo = $c->get(TaskRepositoryInterface::class); //автоматическое внедрение зависимости
    return new TaskController($repo);
});

$controller = $container->get(TaskController::class); //маршруты
